...
from models.diffusion_networks.diffusion_util import (
...
class DDIMSampler(object):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True):
        ...
        alphas_cumprod = self.model.alphas_bar
        assert alphas_cumprod.shape[0] == self.ddpm_num_timesteps, 'alphas have to be defined for each timestep'
        to_torch = lambda x: x.clone().detach().to(torch.float32).to(self.model.device)

        self.register_buffer('betas', to_torch(self.model.betas))
        self.register_buffer('alphas_cumprod', to_torch(alphas_cumprod))
        self.register_buffer('alphas_cumprod_prev', to_torch(self.model.alphas_bar_prev))
    ...
    def sample(self,
        ...
        # sampling
        if self.model.volume_type == 'triplane':
            C, D, H, W = [shape[0], shape[1], shape[1], shape[1]] # TODO: CHECK DIMENSION
            size = (batch_size, C, H+D, W+D)
        elif len(shape) == 4:
            C, D, H, W = shape
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      mm_cls_free=False, use_clamp=True, clamp_scale=5, scale_factor=1,
                      mean_var=False,
                      ):
        ...
        for i, step in enumerate(iterator):
            ...
            if use_clamp and mean_var == False:
                img = torch.clamp(img, -clamp_scale, clamp_scale)
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      ...
                      use_clamp=True, clamp_scale=5, scale_factor=1, mean_var=False):
        ...
        alphas = self.model.alphas_bar if use_original_steps else self.ddim_alphas
        alphas_prev = self.model.alphas_bar_prev if use_original_steps else self.ddim_alphas_prev
        ...
        if use_clamp and mean_var == False:
            pred_x0 = torch.clamp(pred_x0, -scale_factor * 1.01, scale_factor * 1.01)

        
        # print("[DEBUG] MAX/MIN of Pred_x0 & present x:", torch.max(pred_x0).item(), torch.min(pred_x0).item(), torch.max(x).item(), torch.min(x).item())
        # e_t = x - pred_x0 * a_t.sqrt() / sqrt_one_minus_at
